...
class HeartRateDailyView extends StatefulWidget {
  final String userId;
  const HeartRateDailyView({super.key, required this.userId});

  @override
  State<HeartRateDailyView> createState() => _HeartRateDailyViewState();
}

class _HeartRateDailyViewState extends State<HeartRateDailyView> {
  ...
  Widget _buildChart(
    ...
    // Dynamically compute min/max from actual readings with ±10 BPM padding,
            ...
            LineChartBarData(
              ...
              dotData: FlDotData(
                ...
                getDotPainter: (spot, p, bar, idx) => FlDotCirclePainter(
                  radius: 3,
                  color: _getHeartRateColor(spot.y),
                  strokeWidth: 1.5,
  ...
  Widget _buildLogItem(
    ...
    final color = _getHeartRateColor(item.value);
    final source = item.data?['source'] as String? ?? '';
    final isFromAllowear = source == 'allowear';

    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: isFromAllowear ? null : () => _editHeartRate(item),
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            Text(
              item.value > 100 ? 'High' : (item.value < 60 ? 'Low' : 'Normal'),
              style: TextStyle(
                fontSize: 10,
                fontWeight: FontWeight.w800,
                color: color,
  ...
  Future<void> _editHeartRate(VitalsStreamResponse item) async {
    final updated = await showHeartRateEntryDialog(context, item: item);
    if (updated == true) {
      await _fetchData();
    }
  }

  Future<void> _deleteHeartRate(VitalsStreamResponse item) async {
