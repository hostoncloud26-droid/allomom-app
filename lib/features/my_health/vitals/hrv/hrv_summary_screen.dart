...
import 'package:allomom/features/my_health/vitals/hrv/views/hrv_daily_view.dart';
import 'package:allomom/features/my_health/vitals/hrv/views/hrv_weekly_view.dart';
import 'package:allomom/features/my_health/vitals/hrv/views/hrv_monthly_view.dart';

class HrvSummaryScreen extends StatefulWidget {
  const HrvSummaryScreen({super.key});

  @override
  State<HrvSummaryScreen> createState() => _HrvSummaryScreenState();
}

class _HrvSummaryScreenState extends State<HrvSummaryScreen> {
  final HealthVitalsController _controller = HealthVitalsController.instance;
  final RxInt _selectedTab = 0.obs;
  int _refreshTick = 0;

  Future<void> _openEntryDialog() async {
    final saved = await showHrvEntryDialog(context);
    if (saved == true) {
  ...
  Widget _buildSelectedView(String userId) {
    ...
    switch (_selectedTab.value) {
      ...
      case 1:
        return HrvWeeklyView(
          key: ValueKey('weekly-$_refreshTick'),
          userId: userId,
        );
      case 2:
        return HrvMonthlyView(
